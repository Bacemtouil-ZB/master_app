    ...
    stages {
        ...
        stage('Scan Server Image') {
            steps {
                script {
                    bat """
                    docker run --rm -v //var/run/docker.sock:/var/run/docker.sock aquasec/trivy:latest image ${IMAGE_NAME_SERVER}:latest
                    """
                }
            }
        }
        ...
        stage('Scan Client Image') {
            steps {
                script {
                    bat """
                    docker run --rm -v //var/run/docker.sock:/var/run/docker.sock aquasec/trivy:latest image ${IMAGE_NAME_CLIENT}:latest
                    """
                }
            }
        }
